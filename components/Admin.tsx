...
const UsersTable: React.FC<{ users: Usuario[], onEdit: (u: Usuario) => void, onDelete: (u: Usuario) => void, onBulkDelete: (ids: any[]) => void, canEdit: boolean }> = ({ users, onEdit, onDelete, onBulkDelete, canEdit }) => {
    ...
    return (
        <div>
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-3 w-10 text-center"><Checkbox checked={selectedIds.size > 0 && selectedIds.size === sortedData.length} onChange={toggleAll} /></th><SortableHeader label="Nombre" sortKey="nombre" currentSort={sortConfig} onSort={requestSort} /><SortableHeader label="Email" sortKey="email" currentSort={sortConfig} onSort={requestSort} /><SortableHeader label="Rol" sortKey="rol" currentSort={sortConfig} onSort={requestSort} />{canEdit && <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Acciones</th>}</tr></thead>
                    <tbody className="bg-white divide-y divide-gray-200">{sortedData.length > 0 ? sortedData.map(u => (<tr key={u.usuario_id} className={`hover:bg-gray-50 cursor-pointer ${selectedIds.has(u.usuario_id!) ? 'bg-blue-50' : ''}`} onClick={() => toggleSelection(u.usuario_id!)}><td className="px-4 py-4 text-center"><Checkbox checked={selectedIds.has(u.usuario_id!)} onChange={() => toggleSelection(u.usuario_id!)} /></td><td className="px-4 py-4 text-sm text-gray-900 font-medium">{u.nombre}</td><td className="px-4 py-4 text-sm text-gray-500">{u.email}</td><td className="px-4 py-4 text-sm text-gray-500"><span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">{u.rol || 'Consulta'}</span></td>{canEdit && <ActionButtons onEdit={() => onEdit(u)} onDelete={() => onDelete(u)} />}</tr>)) : <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">No se encontraron usuarios.</td></tr>}</tbody>
                </table>
            </div>
        </div>
    );
